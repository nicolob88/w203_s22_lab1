...
2) The async suggested that the best effect size for this is the correlation effect size $r=\frac{Z}{\sqrt{N}}$. Which is also very small:
...
3) We can also calculate Cohen's D metric (this requires the `effsize` package). This specifies how many standard deviations away from the population is one distribution from another one. Again, very small:
